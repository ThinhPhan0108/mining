...
class ComBine:
    def __init__(self):
        ...
        self.wks=gc.open('Auto Alpha').worksheet('auto_alpha')
        self.wks_combine=gc.open('Auto Alpha').worksheet('combine')

        #lấy dữ liệu
        '''df_alpha_history=self.wks.get_all_records()
        df_alpha_history=pd.DataFrame(df_alpha_history)
        #xử lý dữ liệu
        df_alpha_history=self.procesing_data_alpha(df_alpha_history)
        #lọc dữ liệu
        self.df_alpha_history=df_alpha_history[df_alpha_history['Sharpe']>=0.5].reset_index(drop=True)'''
        
        self.main_database=pd.read_csv('./combine/main.csv')
        self.wl=WorldQuant()
    
    #xử lý data
    def procesing_data_alpha(self,data):
        #lấy những dữ liệu cần thiết
        data=data[['update', 'process type', 'source', 'Group Hypothesis','Sub Hypothesis', 'Description', 'Expression', 'Alpha', 'Sharpe','Turnover', 'Fitness', 'Returns', 'Drawdown', 'Margin','weight','sub_univese','settings','min_corr','max_corr','score']].copy()
        #đổi kiểu dữ liệu
        columns_list=['Sharpe','Turnover','Fitness','Returns','Drawdown','Margin']
    ...
    def get_code(self,alpha: str,setting: str):
        ...
        if id in list(self.main_database['id']):
            ...
            #bổ sung alpha
            result_simulate=[id,alpha]+result_simulate
            #thêm vào database
            self.main_database.loc[len(self.main_database)]=result_simulate
            ...
            self.main_database.to_csv('./combine/main.csv',index=False)
            return code
    ...
    def run_v2(self):
        ...
        #chạy tổ hợp code
        for code1,code2 in map_codes:
            try:
                pl_code1=pd.read_csv(f'./combine/details/{code1}.csv')
                pl_code1=pl_code1[['date','returns']].iloc[:973] #chỉ lấy phần train

                pl_code2=pd.read_csv(f'./combine/details/{code2}.csv')
                ...
                alpha=self.expression_combine([code1,code2],weights)
                #simulate
                result_simulate=self.wl.single_simulate(alpha,neut="NONE")
                #xuất kết quả
                results=[code1,code2,str(weights),alpha,sharpe_max]+result_simulate
                print(results)
    ...
    def run(self,alpha,setting):
        ...
        for code_1,code_2 in maps:
            try:
                ...
                #simulate
                result_simulate=self.wl.single_simulate(alpha_combine,neut="NONE")
