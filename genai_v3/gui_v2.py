...
# ========== SIMULATION CONTROL METHODS ==========
    ...
    def run_simulation(self):
        ...
        try:
            for i, alpha in enumerate(self.selected_alphas):
                ...
                self.root.after(0, lambda idx=i, total=len(self.selected_alphas), a=alpha: self.update_simulation_progress(
                    f"Simulating alpha {idx+1}/{total}: {a[:30]}..."))
                            
                try:
                    self.wq.simulate([alpha])
                    self.root.after(0, lambda v=i+1: self.simulation_progress.config(value=v))
                    time.sleep(0.5)
                                
                except Exception as e:
                    print(f"Error simulating alpha {i+1}: {e}")
                    continue
