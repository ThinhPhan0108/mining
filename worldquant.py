...
class WorldQuant:
    ...
    def flow_simulate(self,simulation_progress_url_list,results,number_flow):
        if len(simulation_progress_url_list)==number_flow:
            ...
            while True:
                ...
                if simulation_progress.get("status") in ['COMPLETE','WARNING']:
                    alpha_id=simulation_progress.get("alpha")
                    result=self.locate_alpha(alpha_id,True) #lấy giá trị
                    print(result)
                    #đọc file data alpha và lưu dữ liệu dữ liệu
                    data_alpha=pd.read_csv('./genai_v3/data_alpha.csv')
                    data_alpha.loc[len(data_alpha)] = result
                    data_alpha.to_csv('./genai_v3/data_alpha.csv',index=False)

                    #results.append(result)
                    simulation_progress_url_list.pop(i) #xóa khỏi danh sách theo dõi
                    break
                ...
                elif simulation_progress.get("detail")=="Incorrect authentication credentials.":
    ...
    #simulate alpha
    def simulate(self, alpha_data: list,decay=0,truncation=0 ,neut="MARKET",region='USA',universe='TOP3000'):
        """
        ...
        for sim_data in sim_data_list:
            try:
                ...
                sleep(180)  # Short sleep on error
                self.setup_auth(self.credentials_path)
                continue
