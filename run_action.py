...
def run_automation():
    ...
    # Đọc cấu hình từ config.json (nếu có) hoặc dùng giá trị mặc định
    try:
    ...
    try:
        ...
        # --- 2. Đọc và Tạo Alphas ---
        print("\n2. Đọc và tạo các biến thể Alpha...")
        records = alphas_sheet.get_all_records()
        if not records:
            print("   -> Không có alpha nào trong tab 'Alphas'. Kết thúc.")
            return
        ...
        for record in records:
            ...
            print(f"\n   -> Đang tạo biến thể cho: '{base_alpha}' với options: {option_items}")
            generated_list = optimizer.complete_search(base_alpha, option_items)
            all_generated_alphas.extend(generated_list)
            print(f"      => Tạo thành công {len(generated_list)} biến thể. Tổng số hiện tại: {len(all_generated_alphas)}")
        ...
        print(f"\n   -> HOÀN TẤT TẠO BIẾN THỂ. Tổng cộng có {total_alphas} alpha sẽ được simulate.")
