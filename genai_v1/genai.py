...
#from optimize.src_v2 import Optimize 
import csv
...
class GenAI:
    def __init__(self,group_index_key=0,sub_index_key=0,alpha_index_key=0):
        ...
        self.group_prompt=open("./genai_v1/prompt/group_hypothesis_prompt.txt", "r", encoding="utf-8").read()

        self.sub_prompt=open("./genai_v1/prompt/sub_hypothesis_prompt.txt", "r", encoding="utf-8").read()
        #self.sub_system=open("./genai_v1/prompt/sub_hypothesis_system.txt", "r", encoding="utf-8").read()
        ...
        wks = gc.open("Finding Alpha").worksheet("Auto_alpha_demo_v2")
        self.wks=wks

        group_wks = gc.open("Finding Alpha").worksheet("Group Hypothesis")
                ...
                for j in range(len(df_sub_hypothesis)):
                    ...
                    if expression_alpha != 'invalid':
                        ...
                        if sharpe and sharpe >=0:
                            ...
                            auto_alpha['Expression_alpha']=expression_alpha #cập nhật lại Expression_alpha trong auto_alpha 
                            
                            result_simulate=wl.single_simulate(expression_alpha)
                            results=[[self.date, self.process_name, file_name]+auto_alpha.values.tolist()[0]+result_simulate]
                        
                        else: #sharpe không tồn tại
...
if __name__ == '__main__':
    ...
    while True:
        try:
            GenAI(group_index_key,sub_index_key,alpha_index_key).run(file_pdf_path,file_sub_hypothesis_path)
        except Exception as e:
